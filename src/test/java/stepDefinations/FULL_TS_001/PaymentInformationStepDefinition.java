...
public class PaymentInformationStepDefinition {
    ...
    @Then("Click on the eye icon for product details")
    public void clickOnTheEyeIconForProductDetails() throws InterruptedException, InvalidMidiDataException {

        ReusableMethods.wait(3);
       ReusableMethods.koordinatTiklamaMethoduOguz(1252, 707);
        ReusableMethods.wait(2);
        ReusableMethods.dikeyKaydirma(Driver.getAppiumDriver(),0.70,0.30,0.50,100);

    }

    @And("Goes to the invoice page by clicking the Receipt details button")
    public void goesToTheInvoicePageByClickingTheReceiptDetailsButton() {

        paymentInformationPage.receiptDetails.click();
        ReusableMethods.wait(3);
    }


    @And("Confirms that it is on the invoice page")
    public void confirmsThatItIsOnTheInvoicePage() {

        Assert.assertTrue(paymentInformationPage.printInvoiceButton.isDisplayed());

    }

    @And("Logs out of the invoice page")
    public void logsOutOfTheInvoicePage() {

        paymentInformationPage.backButton.click();
    }
}
